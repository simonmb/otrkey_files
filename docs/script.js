(async function () {
  ...
  let files = []; // available inside IIFE

  // ----------------------------
  // Helpers: download zip w/ progress
  // ----------------------------
  ...
  const mirrorList = await fetch(
    `https://cdn.jsdelivr.net/gh/simonmb/otrkey_files@main/mirrors.json?${CB}`
  ).then((r) => r.json());
  ...
  // keep cutlist count (n_cutlists) and all_errors from CSV
  files = parsedCsv.data.map((row) => ({
    ...row,
    cutlistCount: Number(row.n_cutlists) || 0,
    all_errors: (row.all_errors || "").trim(),
    parsed: parseOtrkeyFilename(row.file_name),
  }));

  // Build mirror map from name -> search_url
  const mirrorMap = {};
  ...
  document
    .querySelectorAll('#formatFilter input[type="checkbox"]')
  ...
  async function performSearch() {
    ...
    const selectedFormats = Array.from(
      document.querySelectorAll("#formatFilter input:checked")
    ).map((i) => i.value.toLowerCase());

    const selectedMirrors = Array.from(
      document.querySelectorAll("#mirrorFilter input:checked")
    ).map((i) => i.value);
    ...
    const filtered = files.filter(({ parsed, file_name, mirror_name }) => {
      ...
      const matchesMirror =
        selectedMirrors.length === 0 || selectedMirrors.includes(mirror_name);
    ...
    for (const row of filtered) {
      ...
      groups.get(p.sortKey).push({
        ...
        all_errors: row.all_errors, // <- keep per-entry error string
      });
    }
    ...
    for (const [, filesInGroup] of sortedGroups) {
      const { title, date, time, channel, duration, season, episode } =
        filesInGroup[0].parsed;
      ...
      const maxCutlists = Math.max(
        ...filesInGroup.map((f) => f.cutlistCount || 0)
      );
      ...
      clIcon.setAttribute("aria-hidden", "true");
      clIcon.textContent = "🎬";

      if (maxCutlists > 0) {
        clIcon.title = "cutlist available: click to recheck...";
      } else {
        clIcon.title = "cutlist unavailable: click to recheck...";
        clIcon.style.opacity = "0.4";
      }

      cutlistLink.appendChild(clIcon);

      // attach meta + cutlist link inline
      headingDiv.appendChild(metaDiv);
      headingDiv.appendChild(cutlistLink);

      // --- NEW: error icon next to cutlist if any entry in this group has errors ---
      const errorStrings = Array.from(
        new Set(
          filesInGroup
            .map((f) => (f.all_errors || "").trim())
            .filter((s) => s.length > 0)
        )
      );
      ...
      if (errorStrings.length > 0) {
        ...
        errorIcon.title = errorStrings.join(" | ");
        errorIcon.textContent = " ⚠️"; // leading space for spacing
        errorIcon.className = "small ms-1";
        headingDiv.appendChild(errorIcon);
      }
  ...
  function parseOtrkeyFilename(filename) {
    const pattern =
      /^(?<title>.+?)(?:_S(?<season>\d{2})E(?<episode>\d{2}))?_(?<date>\d{2}\.\d{2}\.\d{2})_(?<time>\d{2}-\d{2})_(?<channel>[a-z0-9]+)_(?<duration>\d+)_TVOON_DE\.mpg(?:\.(?<quality>HQ|HD))?(?:\.fra)?(?:\.auto)?(?:\.cut)?\.(?:(?<video_format>avi|mp4)|(?<audio_format>ac3|mp3))\.otrkey$/i;
    ...
    if (info.date) {
      ...
      // sortKey used for grouping+sorting (kept as-is from original)
